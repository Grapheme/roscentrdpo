					...
					<?php for($i=0;$i<count($orders);$i++):?>
						<tr>
							...
							<td style="max-width:135px;">
								...
								<nobr>
								<?php if($orders[$i]['has_protokol'] === TRUE):?>
									<?=anchor('admin-panel/messages/orders/id/'.$orders[$i]['id'].'/protokol','<img src="'.$baseurl.'img/icon/blog-blue.png" />',array('target'=>'_blank','title'=>'Протокол итогового тестирования'));?>&nbsp;
								<?php else:?>
									<?=anchor('admin-panel/messages/orders/id/'.$orders[$i]['id'].'/statement','<img src="'.$baseurl.'img/icon/blog-blue.png" />',array('target'=>'_blank','title'=>'Ведомость итогового тестирования'));?>&nbsp;
								<?php endif;?>
