...
//			$this->testsmodel->delete_record($test);
	...
	public function orders_messages(){
		...
		endswitch;
		if($this->uri->total_segments() >= 6):
			$pagevar['pages'] = $this->pagination('admin-panel/messages/orders/'.$this->uri->segment(4).'/'.$field.'/'.$sortby,7,$pagevar['count'],5);
		else:
			$pagevar['pages'] = $this->pagination('admin-panel/messages/orders/'.$this->uri->segment(4),5,$pagevar['count'],5);
		endif;
		...
		for($i=0;$i<count($pagevar['orders']);$i++):
			$pagevar['orders'][$i]['has_protokol'] = FALSE;
			if(!empty($pagevar['orders'][$i]['numbercompletion']) && strtotime($pagevar['orders'][$i]['closedate']) >= 1388520000):
				$pagevar['orders'][$i]['has_protokol'] = TRUE;
			endif;
...
//		print_r($pagevar['orders']);exit;
	...
	public function orders_paid(){
		...
		if(!$order = $this->input->post('order')) show_404();
		if(!$access = $this->input->post('access')) $access = 0;
	...
	public function orders_send_mail(){
		...
		$info['userpaiddate'] = $this->operation_dot_date($info['userpaiddate']);
		ob_start();
		?>
			<html><body>
			<img src="<?=base_url('img/logo_small.png')?>" alt="" /><br/>
		...
		if($smtype == 'smtext'):
			...
			$mailSubject = 'Уведомление об оплате за обучение';
			?>
			<p>
			...
			<p>
				<em><strong>Ваш период  обучения с <?=$info['userpaiddate'];?> по <?=date("d.m.Y",future_days(COUNT_DAY_EXAMEN,$info['closedate']));?></em></strong><br/>
				<em><strong>Доступ для сдачи итогового тестирования Вам будет доступен с <?=$info['closedate'];?> по <?=date("d.m.Y",future_days(COUNT_DAY_EXAMEN,$info['closedate']));?></em></strong><br/>
	...
	public function protokol(){
		...
		$pagevar = array(
			...
			'baseurl' 		=> base_url(),
			'userinfo'		=> $this->user,
			'datebegin'		=> $this->ordersmodel->read_field($this->uri->segment(5),'paiddate'),
			'dateend'		=> $this->ordersmodel->read_field($this->uri->segment(5),'closedate'),
			'hours'			=> 0,
			'courses'		=> $this->unionmodel->read_course_audience_records($this->uri->segment(5)),
			'customer'		=> $this->unionmodel->customerByOrderID($this->uri->segment(5),'organization'),
			'ncompletion'	=> $this->ordersmodel->read_field($this->uri->segment(5),'numbercompletion'),
		);
		...
		$this->load->view("admin_interface/documents/protokol",$pagevar);
	}
	...
	public function completion(){
		...
		$pagevar = array(
			...
			'baseurl' 		=> base_url(),
			'userinfo'		=> $this->user,
			'datebegin'		=> $this->ordersmodel->read_field($orderID,'paiddate'),
			'dateend'		=> $this->ordersmodel->read_field($orderID,'closedate'),
			'hours'			=> 0,
			'ncompletion'	=> $this->ordersmodel->read_field($orderID,'numbercompletion'),
			'courses'		=> $this->unionmodel->read_course_audience_records($orderID),
			'customer'		=> $this->unionmodel->customerByOrderID($orderID,'organization'),
		);
		if($pagevar['datebegin']!='0000-00-00'):
			$pagevar['datebegin'] = preg_split("/[ ]+/",$this->operation_date($pagevar['datebegin']));
		else:
	...
	public function admission(){
		...
		$pagevar = array(
			...
			'userinfo'		=> $this->user,
			'datebegin'		=> $this->ordersmodel->read_field($orderID,'paiddate'),
			'nplacement'	=> $this->ordersmodel->read_field($orderID,'numberplacement'),
			'courses'		=> $this->unionmodel->read_course_audience_records($orderID),
			'customer'		=> $this->unionmodel->customerByOrderID($orderID,'organization'),
		);
		if($pagevar['datebegin']!='0000-00-00'):
	...
	public function act(){
		...
		if($pagevar['order']['closedate'] != "0000-00-00"):
			$pagevar['order']['closedate_original'] = $pagevar['order']['closedate'];
			$pagevar['order']['closedate'] = $this->operation_date($pagevar['order']['closedate']);
		else:
			$pagevar['order']['closedate_original'] = '';
	...
	public function users_customer(){
		...
		for($i=0;$i<count($pagevar['customers']);$i++):
			...
			$pagevar['customers'][$i]['signupdate'] = $this->operation_dot_date($pagevar['customers'][$i]['signupdate']);
		endfor;
	...
	public function users_audience(){
		...
		for($i=0;$i<count($pagevar['audience']);$i++):
			...
			$pagevar['audience'][$i]['signupdate'] = $this->operation_dot_date($pagevar['audience'][$i]['signupdate']);
		endfor;
	...
	public function fiz_protokol(){
		...
		$pagevar = array(
			...
			'ncompletion'	=> $this->fizordersmodel->read_field($order,'numbercompletion'),
			'courses'		=> $this->fizunionmodel->read_course_physical_records($order),
			'ncompletion'	=> $this->fizordersmodel->read_field($this->uri->segment(5),'numbercompletion'),
		);
		if($pagevar['ncompletion']):
			$pagevar['ncompletion'] = preg_replace("([^0-9\/])","",$pagevar['ncompletion']);
		endif;
		...
		$this->load->view("admin_interface/physical/documents/protokol",$pagevar);
	}
