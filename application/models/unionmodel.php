...
class Unionmodel extends CI_Model {
	...
	function read_course_audience_records($order){
		...
		$query = "SELECT audienceorder.*,audience.lastname,audience.name,audience.middlename,audience.fiodat,audience.specialty,audience.position,courses.code AS ccode,courses.title AS ctitle,courses.hours AS chours FROM audienceorder INNER JOIN audience ON audienceorder.audience=audience.id INNER JOIN courseorder ON audienceorder.course=courseorder.id, courses WHERE audienceorder.order = $order AND courseorder.course = courses.id ORDER BY audienceorder.id";
		$query = $this->db->query($query);
		$data = $query->result_array();
		if(count($data)) return $data;
		return NULL;
	...
	function read_fullinfo_audience($order){
		...
		$query = "SELECT audienceorder.*,audience.lastname,audience.name,audience.middlename,audience.fiodat,audience.specialty,audience.position,audience.personaphone,audience.address,customers.organization,customers.inn,customers.kpp,customers.uraddress,orders.id AS ordid,orders.orderdate,orders.userpaiddate,orders.price AS ordprice,orders.paid,orders.paiddate,orders.discount,orders.docnumber,courses.code AS ccode,courses.title AS ctitle,courses.hours AS chours,courseorder.price AS сprice,courses.price AS manual_price FROM audienceorder INNER JOIN audience ON audienceorder.audience=audience.id INNER JOIN courseorder ON audienceorder.course=courseorder.id INNER JOIN customers ON audienceorder.customer = customers.id INNER JOIN orders ON audienceorder.order = orders.id, courses WHERE audienceorder.order = $order AND courseorder.course = courses.id ORDER BY audienceorder.id";
		$query = $this->db->query($query);
		$data = $query->result_array();
		if(count($data)) return $data;
		return NULL;
	...
	function read_courseorder_title($order){
		...
		$query = "SELECT courses.code, courses.title, COUNT(audienceorder.id) as audience FROM courseorder INNER JOIN courses ON courseorder.course=courses.id LEFT JOIN audienceorder ON courseorder.course = audienceorder.course WHERE courseorder.order = $order ORDER BY courseorder.id";
		$query = $this->db->query($query);
		$data = $query->result_array();
		if(count($data)) return $data;
		return NULL;
	...
	function customerByOrderID($orderID,$field = NULL){
		
		$query = "SELECT orders.*,customers.id AS cid,customers.organization,customers.personemail,customers.phones,customers.online FROM orders INNER JOIN customers ON orders.customer = customers.id WHERE orders.id = $orderID";
		if($data = $this->db->query($query)->result_array()):
			if(is_null($field)):
				return $data[0];
			elseif(!is_null($field) && isset($data[0][$field])):
				return $data[0][$field];
			endif;
		endif;
		return NULL;
