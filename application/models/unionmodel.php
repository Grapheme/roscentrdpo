...
class Unionmodel extends CI_Model {
	...
	function read_fullinfo_audience($order){
		...
		$query = "SELECT audienceorder.*,audience.lastname,audience.name,audience.middlename,audience.fiodat,audience.specialty,audience.position,audience.personaphone,customers.organization,customers.inn,customers.kpp,customers.uraddress,orders.id AS ordid,orders.orderdate,orders.userpaiddate,orders.price AS ordprice,orders.paid,orders.paiddate,orders.discount,orders.docnumber,courses.code AS ccode,courses.title AS ctitle,courses.hours AS chours,courseorder.price AS сprice,courses.price AS manual_price FROM audienceorder INNER JOIN audience ON audienceorder.audience=audience.id INNER JOIN courseorder ON audienceorder.course=courseorder.id INNER JOIN customers ON audienceorder.customer = customers.id INNER JOIN orders ON audienceorder.order = orders.id, courses WHERE audienceorder.order = $order AND courseorder.course = courses.id ORDER BY audienceorder.id";
		$query = $this->db->query($query);
		$data = $query->result_array();
		if(count($data)) return $data;
		return NULL;
